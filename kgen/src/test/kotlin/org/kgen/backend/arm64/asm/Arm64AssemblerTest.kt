...
class Arm64AssemblerTest {
    ...
    fun `FP LDR D0 from SP offset 16 encodes correctly`() {
        val bytes = assemble { fldr(D0, Arm64Register.SP as Arm64Register64, 16) }
    ...
    fun `FP STR D0 to SP offset 0 encodes correctly`() {
        val bytes = assemble { fstr(D0, Arm64Register.SP as Arm64Register64, 0) }
        assertEquals(0xFD0003E0L, readLE32(bytes))
    }

    @Test
    fun `FP STP pre-index encodes correctly`() {
        val bytes = assemble { fstpPre(D0, D1, Arm64Register.SP as Arm64Register64, -16) }
