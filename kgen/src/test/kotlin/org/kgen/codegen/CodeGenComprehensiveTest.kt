...
class CodeGenComprehensiveTest {
    ...
    fun `condBr with phi compiles on all targets`() {
        compileOnAllExcept("wasm") {
            ...
            val result = phi(Type.I32, listOf(thenVal to "then", elseVal to "else"))
            ret(result)
            finalizeFunction()
        }
    }
    ...
    fun `diamond cfg with arithmetic compiles on all targets`() {
        compileOnAllExcept("wasm") {
            ...
            positionAtEnd(appendBlock("entry"))
            val cond = icmp(ICmpPredicate.SGT, params[0], params[1])
            condBr(cond, "a_gt", "b_gt")
            positionAtEnd(appendBlock("a_gt"))
            val diffA = sub(params[0], params[1])
            br("done")
            positionAtEnd(appendBlock("b_gt"))
            val diffB = sub(params[1], params[0])
            br("done")
            positionAtEnd(appendBlock("done"))
            val result = phi(Type.I32, listOf(diffA to "a_gt", diffB to "b_gt"))
            ret(result)
            finalizeFunction()
        }
    }
