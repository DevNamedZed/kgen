...
class PipelineExtendedTest {
    ...
    private fun build(target: Target = Target.x86_64(), block: ModuleBuilder.() -> Unit): Module {
    ...
    fun singlePipeline() {
        ...
        val result = Pipeline().add(ConstantFolding()).execute(module)
    ...
    fun jumpThreadingMultipleBranches() {
        ...
        val result = Pipeline().add(JumpThreading()).execute(module)
    ...
    fun foldAndJumpThreadingChain() {
        val module = build {
            ...
            condBr(cond, BlockRef("yes"), BlockRef("no"))
    ...
    fun allDeadCodeRemoved() {
        ...
        val result = Pipeline().add(DeadCodeElimination()).execute(module)
    ...
    fun passOrderMattersForOptimization() {
        ...
        // Fold first, then DCE
        val r1 = Pipeline().add(ConstantFolding()).add(DeadCodeElimination()).execute(module)
        assertEquals(1, r1.functions[0].blocks[0].instructions.size, "Fold+DCE: only ret")

        // DCE first, then fold
        val r2 = Pipeline().add(DeadCodeElimination()).add(ConstantFolding()).execute(module)
    ...
    fun moduleNamePreserved() {
        val ir = ModuleBuilder("my_module", Target.x86_64())
