...
class WasmControlFlowTest {
    ...
    fun `br forward skip`() {
        ...
        ir.positionAtEnd(ir.appendBlock("entry"))
        ir.br("exit")
        ir.positionAtEnd(ir.appendBlock("middle"))
    ...
    fun `condbr if-then-else diamond`() {
        ...
        val phi = ir.phi(Type.I32, listOf(params[0] to "then", params[1] to "else"))
        ir.ret(phi)
        ir.finalizeFunction()
        assertValidWasm(gen.generate(ir.build()))
    }
    ...
    fun `condbr neither target is next block`() {
        ...
        ir.positionAtEnd(ir.appendBlock("dead"))
    ...
    fun `switch with multiple cases`() {
        ...
        ir.positionAtEnd(ir.appendBlock("one"))
        ir.ret(Constant.I32(20))
        ir.positionAtEnd(ir.appendBlock("two"))
    ...
    fun `phi with two incoming values`() {
        ...
        ir.condBr(isTrue, "left", "right")
        ir.positionAtEnd(ir.appendBlock("left"))
        ir.br("merge")
        ir.positionAtEnd(ir.appendBlock("right"))
        ir.br("merge")
        ir.positionAtEnd(ir.appendBlock("merge"))
        val result = ir.phi(Type.I32, listOf(params[1] to "left", params[2] to "right"))
        ir.ret(result)
        ir.finalizeFunction()
        assertValidWasm(gen.generate(ir.build()))
    }
    ...
    fun `multiple returns in different blocks`() {
        ...
        ir.positionAtEnd(ir.appendBlock("entry"))
        val isZero = ir.icmp(ICmpPredicate.EQ, params[0], Constant.I32(0))
        ir.condBr(isZero, "retZero", "check")

        ir.positionAtEnd(ir.appendBlock("retZero"))
        ir.ret(Constant.I32(0))

        ir.positionAtEnd(ir.appendBlock("check"))
        val isOne = ir.icmp(ICmpPredicate.EQ, params[0], Constant.I32(1))
        ir.condBr(isOne, "retOne", "retDefault")

        ir.positionAtEnd(ir.appendBlock("retOne"))
        ir.ret(Constant.I32(1))

        ir.positionAtEnd(ir.appendBlock("retDefault"))
        ir.ret(Constant.I32(-1))
        ir.finalizeFunction()
        assertValidWasm(gen.generate(ir.build()))
    }
    ...
    fun `stackifier orders blocks correctly`() {
        ...
        ir.positionAtEnd(ir.appendBlock("entry"))
        ir.br("a")
        ir.positionAtEnd(ir.appendBlock("a"))
        ir.br("b")
        ir.positionAtEnd(ir.appendBlock("b"))
        ir.ret(Constant.I32(0))
        ir.finalizeFunction()
