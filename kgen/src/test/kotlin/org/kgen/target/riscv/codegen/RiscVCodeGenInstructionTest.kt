...
class RiscVCodeGenInstructionTest {
    ...
    fun `ret f64 constant`() {
        // getOrLoad doesn't handle Constant.F64 (F64 constants need GP register materialization)
    ...
    @Test
    fun `condBr with all icmp predicates on i64`() {
        for (pred in ICmpPredicate.entries) {
            assertCompiles {
                val p = createFunction("f_${pred.name}", listOf(Param("a", Type.I64), Param("b", Type.I64)), Type.I64)
                appendBlock("entry")
                val c = icmp(pred, p[0], p[1])
                condBr(c, BlockRef("then"), BlockRef("else"))
                appendBlock("then")
                ret(p[0])
                appendBlock("else")
