...
class RiscVTlsCodegenTest {
    ...
    fun tlsLoadGeneratesTprelRelocations() {
        ...
        val code = RiscVCodeGenerator().generateCode(module)
        val hasHi20 = code.relocations.any { it.type == RelocationType.RiscV.TPREL_HI20 }
        val hasLo12 = code.relocations.any { it.type == RelocationType.RiscV.TPREL_LO12_I }
        val hasAdd = code.relocations.any { it.type == RelocationType.RiscV.TPREL_ADD }
        assertTrue(hasHi20, "Should have TPREL_HI20 relocation")
        assertTrue(hasLo12, "Should have TPREL_LO12_I relocation")
        assertTrue(hasAdd, "Should have TPREL_ADD relocation")
    }
    ...
    fun tlsStoreGeneratesTprelRelocations() {
        ...
        val code = RiscVCodeGenerator().generateCode(module)
        val hasHi20 = code.relocations.any {
            it.type == RelocationType.RiscV.TPREL_HI20 && it.symbol == "tls_counter"
        }
        assertTrue(hasHi20, "TLS store should generate TPREL_HI20 relocation")
    }
    ...
    fun tlsLoadEmitsThreeInstructions() {
        ...
        val code = RiscVCodeGenerator().generateCode(module)
        // lui + add + addi = 12 bytes, plus ld + ret + prologue/epilogue
        assertTrue(code.textBytes.size >= 12, "Should have at least 3 TLS + 1 load instructions")
    }
    ...
    fun tlsRelocationsTargetCorrectSymbol() {
        ...
        val tlsRelocs = code.relocations.filter {
            it.type == RelocationType.RiscV.TPREL_HI20 ||
                it.type == RelocationType.RiscV.TPREL_LO12_I ||
                it.type == RelocationType.RiscV.TPREL_ADD
        }
