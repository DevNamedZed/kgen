...
import org.kgen.backend.x86.asm.*
    ...
    private class FunctionEmitter(
        ...
        private fun emitInstruction(inst: Instruction) {
            when (inst) {
                ...
                is Instruction.FAdd -> emitFBinOp(inst.dest, inst.lhs, inst.rhs) { d, s -> asm.addsd(d, s) }
                is Instruction.FSub -> emitFBinOp(inst.dest, inst.lhs, inst.rhs) { d, s -> asm.subsd(d, s) }
                is Instruction.FMul -> emitFBinOp(inst.dest, inst.lhs, inst.rhs) { d, s -> asm.mulsd(d, s) }
                is Instruction.FDiv -> emitFBinOp(inst.dest, inst.lhs, inst.rhs) { d, s -> asm.divsd(d, s) }
        ...
        private fun emitICmp(inst: Instruction.ICmp) {
            ...
            when (inst.lhs.type) {
                ...
                Type.I64, Type.OpaquePointer, is Type.Pointer -> {
                    val lhsReg = getOrLoad64(inst.lhs.name, r10_64)
                    val rhs = inst.rhs
        ...
        private fun emitLoad(inst: Instruction.Load) {
            ...
            when (destType) {
                Type.I32 -> {
                    val d = getDest32(inst.dest.name)
                    val addr = getOrLoad64(ptr.name, r11_64)
                    // mov d, [addr]
        ...
        private fun emitSIToFP(inst: Instruction.SIToFP) {
            ...
            when (inst.value.type) {
                Type.I32 -> {
                    ...
                    asm.cvtsi2sd(d, src as X86Operand32)
                }
                Type.I64 -> {
        ...
        private fun emitFusedCmpBranch(
            ...
            when (cmp.lhs.type) {
                Type.I64, Type.OpaquePointer, is Type.Pointer -> {
                    val lhsReg = getOrLoad64(cmp.lhs.name, r10_64)
                    val rhs = cmp.rhs
                    if (rhs is Constant.I32) {
                    ...
                    val lhsReg = getOrLoad32(cmp.lhs.name, r10d32)
                    val rhs = cmp.rhs
                    if (rhs is Constant.I32) {
                        asm.cmp(lhsReg, rhs.value)
                    } else {
