...
class IrParser private constructor(private val input: String) {
        ...
        return when {
            ...
            tryConsume("managed.call ") -> {
                val direction = if (tryConsume("managed_to_native ")) ManagedCallDirection.MANAGED_TO_NATIVE
                    else { expect("native_to_managed "); ManagedCallDirection.NATIVE_TO_MANAGED }
                val retType = parseType()
                expect(" ")
                val function = parseValue(Type.OpaquePointer, params)
                expect("(")
                val args = mutableListOf<Value>()
                if (!tryConsume(")")) {
                    do {
                        val (_, arg) = parseTypedValue(params)
                        args.add(arg)
                    } while (tryConsume(", "))
                    expect(")")
                }
    ...
    private fun parseInstructionNoDest(params: List<Parameter>): Instruction? {
        return when {
            ...
            tryConsume("gc.write_barrier ") -> {
                ...
                expect(" ")
                val obj = parseValue(objType, params)
                expect(", ")
                val idxType = parseType()
                expect(" ")
                val fieldIndex = parseValue(idxType, params)
                expect(", ")
    ...
    private fun parseType(): Type {
        return when {
            ...
            tryConsume("interiorref<") -> {
                val pointee = parseType()
                expect(">")
                Type.InteriorRef(pointee)
            }
            tryConsume("pinnedref<") -> {
                val referent = parseType()
                expect(">")
                Type.PinnedRef(referent)
            }
            tryConsume("[") -> {
                val size = parseLong()
                expect(" x ")
