...
class Arm64CodeGenerator : CodeGenerator {
    ...
    private class FunctionEmitter(
        ...
        private fun emitInstruction(inst: Instruction) {
            ...
            val i = inst
            when (i) {
                is Add -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.add(rd, rn, rm) }
                is Sub -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.sub(rd, rn, rm) }
                is Mul -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.mul(rd, rn, rm) }
                is SDiv -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.sdiv(rd, rn, rm) }
                is UDiv -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.udiv(rd, rn, rm) }
                is And -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.and_(rd, rn, rm) }
                is Or -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.orr(rd, rn, rm) }
                is Xor -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.eor(rd, rn, rm) }
                is Shl -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.lsl(rd, rn, rm) }
                is LShr -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.lsr(rd, rn, rm) }
                is AShr -> emitBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.asr(rd, rn, rm) }

                is SRem -> emitRem(i.dest, i.lhs, i.rhs, signed = true)
                is URem -> emitRem(i.dest, i.lhs, i.rhs, signed = false)
                ...
                is FAdd -> emitFpBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.fadd(rd, rn, rm) }
                is FSub -> emitFpBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.fsub(rd, rn, rm) }
                is FMul -> emitFpBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.fmul(rd, rn, rm) }
                is FDiv -> emitFpBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.fdiv(rd, rn, rm) }
                is FNeg -> emitFpNeg(i)
                is FCmp -> emitFCmp(i)
                ...
                is Ctlz -> emitUnaryIntOp(i.dest, i.operand) { rd, rn -> asm.clz(rd, rn) }
                is Cttz -> emitCttz(i)
                is Ctpop -> emitCtpop(i)
                is BSwap -> emitUnaryIntOp(i.dest, i.operand) { rd, rn -> asm.rev(rd, rn) }

                is PtrToInt -> emitCopy64(i.dest, i.value)
                is IntToPtr -> emitCopy64(i.dest, i.value)
                is BitCast -> emitCopy64(i.dest, i.value)

                is Sqrt -> emitFpUnaryOp(i.dest, i.operand) { rd, rn -> asm.fsqrt(rd, rn) }
                is Ceil -> emitFpUnaryOpD(i.dest, i.operand) { rd, rn -> asm.frintp(rd, rn) }
                is Floor -> emitFpUnaryOpD(i.dest, i.operand) { rd, rn -> asm.frintm(rd, rn) }
                is Round -> emitFpUnaryOpD(i.dest, i.operand) { rd, rn -> asm.frintn(rd, rn) }
                is FAbs -> emitFpUnaryOp(i.dest, i.operand) { rd, rn -> asm.fabs(rd, rn) }
                is FMin -> emitFpBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.fmin(rd, rn, rm) }
                is FMax -> emitFpBinOp(i.dest, i.lhs, i.rhs) { rd, rn, rm -> asm.fmax(rd, rn, rm) }

                is Fence -> asm.dmb()

                is Unreachable -> asm.udf()
                is Trap -> asm.brk(0)
                is DebugTrap -> asm.brk(0)
                ...
                is VAStart -> emitVAStart(i)
                is VAEnd -> {}
                is VAArg -> emitVAArg(i)
                is VACopy -> emitVACopy(i)
                ...
                is SMin -> emitIntMinMax(i.dest, i.lhs, i.rhs, Arm64Condition.LT)
                is SMax -> emitIntMinMax(i.dest, i.lhs, i.rhs, Arm64Condition.GT)
                is UMin -> emitIntMinMax(i.dest, i.lhs, i.rhs, Arm64Condition.CC) // unsigned lower
                is UMax -> emitIntMinMax(i.dest, i.lhs, i.rhs, Arm64Condition.HI) // unsigned higher
                is Abs -> emitAbs(i)
                is FMA -> emitFMA(i)
                is BitReverse -> emitUnaryIntOp(i.dest, i.operand) { rd, rn -> asm.rbit(rd, rn) }
                is Rotl -> emitRotl(i)
                is Rotr -> emitBinOp(i.dest, i.value, i.amount) { rd, rn, rm -> asm.ror(rd, rn, rm) }
                ...
                is Prefetch -> {} // hint only, no-op
                is StackSave -> emitStackSave(i)
                is StackRestore -> emitStackRestore(i)
        ...
        private fun emitStackSave(inst: StackSave) {
        ...
        private fun emitStackRestore(inst: StackRestore) {
        ...
        private fun emitCallBr(inst: CallBr) {
            ...
            for (arg in inst.args) {
                if (arg.type.isFloat()) {
                    ...
                    if (arg.type.is64Bit()) {
                        ...
                        val src = getOrLoad32(arg)
                        val destReg = argRegs32[gpIdx]
                        if (src != destReg) { asm.mov(destReg as Arm64Register32, src as Arm64Register32) }
                    }
                    gpIdx++
                }
            }
            ...
            if (dest != null) {
                if (dest.type.isFloat()) {
                    val destReg = getDestFpD(dest.name)
                    if (destReg != Arm64Register.D0) { asm.fmov(destReg, Arm64Register.D0) }
                    storeToFpD(dest.name, destReg)
                } else if (dest.type.is64Bit()) {
                    val destReg = getDest64(dest.name)
                    if (destReg != Arm64Register.X0) { asm.mov(destReg, Arm64Register.X0) }
        ...
        private fun emitThrow(inst: Throw) {
            val src = getOrLoad64(inst.exception)
            if (src != Arm64Register.X0) {
                asm.mov(Arm64Register.X0, src)
            }
            asm.bl("kgen_throw")
