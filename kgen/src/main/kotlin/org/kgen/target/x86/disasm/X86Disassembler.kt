...
import org.kgen.codegen.Disassembler
...
class X86Disassembler : Disassembler<X86Instruction> {
    ...
    private class DecodeContext(
        ...
        fun decodeInstruction(): X86Instruction? {
            ...
            // VEX 2-byte prefix
            if (b1 == 0xC5) return decodeVex2Byte()

            // VEX 3-byte prefix
            if (b1 == 0xC4) return decodeVex3Byte()

            // EVEX 4-byte prefix
            if (b1 == 0x62) return decodeEvex()
        ...
        // VEX state
        private var vexW = false
        private var vexVvvv = 0
        private var vexL = 0
        private var vexPP = 0
        private var vexMap = 0

        private fun decodeVex2Byte(): X86Instruction? {
            val b = u8()
            rexR = (b and 0x80) == 0; rexX = false; rexB = false
            vexVvvv = ((b shr 3) and 0xF) xor 0xF
            vexL = (b shr 2) and 1
            vexPP = b and 3
            vexW = false
            vexMap = 1 // 0F
            return decodeVexOpcode()
        }

        private fun decodeVex3Byte(): X86Instruction? {
            val b1 = u8(); val b2 = u8()
            rexR = (b1 and 0x80) == 0; rexX = (b1 and 0x40) == 0; rexB = (b1 and 0x20) == 0
            vexMap = b1 and 0x1F
            vexW = (b2 and 0x80) != 0
            vexVvvv = ((b2 shr 3) and 0xF) xor 0xF
            vexL = (b2 shr 2) and 1
            vexPP = b2 and 3
            return decodeVexOpcode()
        }

        private fun decodeEvex(): X86Instruction? {
            val p1 = u8(); val p2 = u8(); val p3 = u8()
            rexR = (p1 and 0x80) == 0; rexX = (p1 and 0x40) == 0; rexB = (p1 and 0x20) == 0
            vexMap = p1 and 0x03
            vexW = (p2 and 0x80) != 0
            vexVvvv = ((p2 shr 3) and 0xF) xor 0xF
            vexPP = p2 and 3
            vexL = (p3 shr 5) and 3 // L'L
            return decodeVexOpcode()
        }

        private fun decodeVexOpcode(): X86Instruction? {
            val opcode = u8()
            val size = if (vexW) 64 else 32

            return when (vexMap) {
                1 -> decodeVexMap0F(opcode, size) // 0F
                2 -> decodeVexMap0F38(opcode, size) // 0F 38
                3 -> decodeVexMap0F3A(opcode, size) // 0F 3A
                else -> fail()
            }
        }

        private fun decodeVexMap0F(opcode: Int, size: Int): X86Instruction? = when (opcode) {
            // vaddps/pd, vsubps/pd, vmulps/pd, vdivps/pd
            0x58 -> decodeVexRegRegRM(if (vexPP == 1) "vaddpd" else "vaddps", size)
            0x5C -> decodeVexRegRegRM(if (vexPP == 1) "vsubpd" else "vsubps", size)
            0x59 -> decodeVexRegRegRM(if (vexPP == 1) "vmulpd" else "vmulps", size)
            0x5E -> decodeVexRegRegRM(if (vexPP == 1) "vdivpd" else "vdivps", size)
            else -> fail()
        }

        private fun decodeVexMap0F38(opcode: Int, size: Int): X86Instruction? = when (opcode) {
            // BMI1
            0xF2 -> { // andn
                val modrm = readModRM()
                val (rm, _) = decodeModRM(modrm, size)
                emit("andn", regOperand(modrm.reg, size), regOperand(vexVvvv, size), rm)
            }
            0xF7 -> { // bextr/sarx/shrx/shlx (differentiated by pp)
                val modrm = readModRM()
                val (rm, _) = decodeModRM(modrm, size)
                val mnem = when (vexPP) {
                    0 -> "bextr"; 1 -> "shlx"; 2 -> "sarx"; 3 -> "shrx"; else -> "bmi?"
                }
                if (vexPP == 0) {
                    emit(mnem, regOperand(modrm.reg, size), rm, regOperand(vexVvvv, size))
                } else {
                    emit(mnem, regOperand(modrm.reg, size), rm, regOperand(vexVvvv, size))
                }
            }
            0xF3 -> { // blsi/blsmsk/blsr (differentiated by ModRM.reg extension)
                val modrm = readModRM()
                val (rm, _) = decodeModRM(modrm, size)
                val mnem = when (modrm.ext) {
                    1 -> "blsr"; 2 -> "blsmsk"; 3 -> "blsi"; else -> "bmi?"
                }
                emit(mnem, regOperand(vexVvvv, size), rm)
            }
            0xF5 -> { // bzhi/pext/pdep (differentiated by pp)
                val modrm = readModRM()
                val (rm, _) = decodeModRM(modrm, size)
                val mnem = when (vexPP) {
                    0 -> "bzhi"; 2 -> "pext"; 3 -> "pdep"; else -> "bmi?"
                }
                if (vexPP == 0) {
                    emit(mnem, regOperand(modrm.reg, size), rm, regOperand(vexVvvv, size))
                } else {
                    emit(mnem, regOperand(modrm.reg, size), regOperand(vexVvvv, size), rm)
                }
            }
            0xF6 -> { // mulx
                val modrm = readModRM()
                val (rm, _) = decodeModRM(modrm, size)
                emit("mulx", regOperand(modrm.reg, size), regOperand(vexVvvv, size), rm)
            }
            else -> fail()
        }

        private fun decodeVexMap0F3A(opcode: Int, size: Int): X86Instruction? = when (opcode) {
            0xF0 -> { // rorx
                val modrm = readModRM()
                val (rm, _) = decodeModRM(modrm, size)
                val imm = readImm8s()
                emit("rorx", regOperand(modrm.reg, size), rm, X86Operand.Immediate(imm.toLong(), 8))
            }
            else -> fail()
        }

        private fun decodeVexRegRegRM(mnemonic: String, size: Int): X86Instruction? {
            val modrm = readModRM()
            val (rm, _) = decodeModRM(modrm, size)
            val suffix = when (vexL) { 0 -> "_x"; 1 -> "_y"; 2 -> "_z"; else -> "" }
            val regName = when (vexL) {
                0 -> "xmm${modrm.reg}"; 1 -> "ymm${modrm.reg}"; 2 -> "zmm${modrm.reg}"; else -> "?${modrm.reg}"
            }
            val vvvvName = when (vexL) {
                0 -> "xmm$vexVvvv"; 1 -> "ymm$vexVvvv"; 2 -> "zmm$vexVvvv"; else -> "?$vexVvvv"
            }
            val rmName = if (rm is X86Operand.Register) {
                when (vexL) {
                    0 -> X86Operand.Register("xmm${(modrm.rm and 0xF)}")
                    1 -> X86Operand.Register("ymm${(modrm.rm and 0xF)}")
                    2 -> X86Operand.Register("zmm${(modrm.rm and 0xF)}")
                    else -> rm
                }
            } else rm
            return emit(mnemonic, X86Operand.Register(regName), X86Operand.Register(vvvvName), rmName)
        }
        ...
        private fun decodeTwoByte(op2: Int): X86Instruction? {
            // SSE/SSE2 instructions dispatched by mandatory prefix
            if (hasRepne) return decodeSseF2(op2)
            if (hasRep) return decodeSseF3(op2)
            if (hasOperandSizeOverride) {
                val sse = decodeSse66(op2)
                if (sse != null) return sse
                // Not SSE — fall through to normal 0x66-prefixed GP instructions
            }

            return decodeTwoByteGP(op2)
        }

        private fun decodeTwoByteGP(op2: Int): X86Instruction? = when (op2) {
            // SYSCALL
            0x05 -> emit("syscall")
        ...
        private fun xmmName(enc: Int): String = "xmm${enc and 0xF}"

        private fun xmmReg(enc: Int): X86Operand.Register = X86Operand.Register(xmmName(enc))

        private fun decodeSseRR(mnemonic: String): X86Instruction? {
            val modrm = readModRM()
            return if (modrm.mod == 3) {
                emit(mnemonic, xmmReg(modrm.reg), xmmReg(modrm.rm))
            } else {
                val (mem, _) = decodeModRM(modrm, 64)
                emit(mnemonic, xmmReg(modrm.reg), mem)
            }
        }

        private fun decodeSseF2(op2: Int): X86Instruction? = when (op2) {
            0x10 -> decodeSseRR("movsd")
            0x11 -> { // movsd rm, xmm (reversed operands)
                val modrm = readModRM()
                if (modrm.mod == 3) {
                    emit("movsd", xmmReg(modrm.rm), xmmReg(modrm.reg))
                } else {
                    val (mem, _) = decodeModRM(modrm, 64)
                    emit("movsd", mem, xmmReg(modrm.reg))
                }
            }
            0x58 -> decodeSseRR("addsd")
            0x5C -> decodeSseRR("subsd")
            0x59 -> decodeSseRR("mulsd")
            0x5E -> decodeSseRR("divsd")
            0x5A -> decodeSseRR("cvtsd2ss")
            0x2A -> { // cvtsi2sd xmm, r/m32 or r/m64
                val modrm = readModRM()
                val sz = if (rexW) 64 else 32
                if (modrm.mod == 3) {
                    emit("cvtsi2sd", xmmReg(modrm.reg), regOperand(modrm.rm, sz))
                } else {
                    val (mem, _) = decodeModRM(modrm, sz)
                    emit("cvtsi2sd", xmmReg(modrm.reg), mem)
                }
            }
            0x2D -> { // cvtsd2si r32/r64, xmm
                val modrm = readModRM()
                val sz = if (rexW) 64 else 32
                if (modrm.mod == 3) {
                    emit("cvtsd2si", regOperand(modrm.reg, sz), xmmReg(modrm.rm))
                } else {
                    val (mem, _) = decodeModRM(modrm, 64)
                    emit("cvtsd2si", regOperand(modrm.reg, sz), mem)
                }
            }
            0x2C -> { // cvttsd2si r32/r64, xmm
                val modrm = readModRM()
                val sz = if (rexW) 64 else 32
                if (modrm.mod == 3) {
                    emit("cvttsd2si", regOperand(modrm.reg, sz), xmmReg(modrm.rm))
                } else {
                    val (mem, _) = decodeModRM(modrm, 64)
                    emit("cvttsd2si", regOperand(modrm.reg, sz), mem)
                }
            }
            else -> fail()
        }

        private fun decodeSseF3(op2: Int): X86Instruction? = when (op2) {
            0x10 -> decodeSseRR("movss")
            0x11 -> {
                val modrm = readModRM()
                if (modrm.mod == 3) {
                    emit("movss", xmmReg(modrm.rm), xmmReg(modrm.reg))
                } else {
                    val (mem, _) = decodeModRM(modrm, 32)
                    emit("movss", mem, xmmReg(modrm.reg))
                }
            }
            0x58 -> decodeSseRR("addss")
            0x5C -> decodeSseRR("subss")
            0x59 -> decodeSseRR("mulss")
            0x5E -> decodeSseRR("divss")
            0x5A -> decodeSseRR("cvtss2sd")
            0x2A -> {
                val modrm = readModRM()
                val sz = if (rexW) 64 else 32
                if (modrm.mod == 3) {
                    emit("cvtsi2ss", xmmReg(modrm.reg), regOperand(modrm.rm, sz))
                } else {
                    val (mem, _) = decodeModRM(modrm, sz)
                    emit("cvtsi2ss", xmmReg(modrm.reg), mem)
                }
            }
            0x2D -> {
                val modrm = readModRM()
                val sz = if (rexW) 64 else 32
                if (modrm.mod == 3) {
                    emit("cvtss2si", regOperand(modrm.reg, sz), xmmReg(modrm.rm))
                } else {
                    val (mem, _) = decodeModRM(modrm, 32)
                    emit("cvtss2si", regOperand(modrm.reg, sz), mem)
                }
            }
            else -> fail()
        }

        private fun decodeSse66(op2: Int): X86Instruction? = when (op2) {
            0x2E -> decodeSseRR("ucomisd")
            0xEF -> decodeSseRR("pxor")
            0x6E -> { // movd/movq xmm, r/m32 or r/m64
                val modrm = readModRM()
                val mnemonic = if (rexW) "movq" else "movd"
                val sz = if (rexW) 64 else 32
                if (modrm.mod == 3) {
                    emit(mnemonic, xmmReg(modrm.reg), regOperand(modrm.rm, sz))
                } else {
                    val (mem, _) = decodeModRM(modrm, sz)
                    emit(mnemonic, xmmReg(modrm.reg), mem)
                }
            }
            0x7E -> { // movd/movq r/m32, xmm or r/m64, xmm
                val modrm = readModRM()
                val mnemonic = if (rexW) "movq" else "movd"
                val sz = if (rexW) 64 else 32
                if (modrm.mod == 3) {
                    emit(mnemonic, regOperand(modrm.rm, sz), xmmReg(modrm.reg))
                } else {
                    val (mem, _) = decodeModRM(modrm, sz)
                    emit(mnemonic, mem, xmmReg(modrm.reg))
                }
            }
            else -> null // Not SSE — return null to fall through to GP decoding
        }
