...
/**
 * Links relocatable ObjectFiles into a Mach-O 64-bit dynamic library (.dylib).
 *
 * Produces a minimal dylib with:
 * - __TEXT segment (__text + __const sections, read-execute)
 * - __DATA segment (__data section, if present, read-write)
 * - LC_ID_DYLIB (library identification)
 * - LC_SYMTAB (symbol table)
 * - LC_DYSYMTAB (dynamic symbol table indices)
 *
 * All defined symbols are exported. Undefined symbols are recorded as imports
 * and left unresolved (resolved at load time by dyld).
 *
 * ```java
 * var linker = new MachODylibLinker(MachO.CPU_TYPE_X86_64, "libfoo.dylib");
 * byte[] dylib = linker.link(List.of(objectFile));
 * ```
 */
class MachODylibLinker(
    private val cpuType: Int = MachO.CPU_TYPE_X86_64,
    private val cpuSubtype: Int = MachO.CPU_SUBTYPE_ALL,
    private val installName: String? = null,
) {
    private companion object {
        ...
        const val ID_DYLIB_CMD_BASE_SIZE = 24
        const val NLIST_SIZE = 16
        const val PAGE_SIZE = 0x1000
        const val TEXT_BASE = 0x0L // dylibs are position-independent, base at 0
    }
    ...
    fun link(objects: List<ObjectFile>): ByteArray {
        ...
        val layout = DylibLayout(merger, symbols, cpuType, installName)
        val relocated = RelocationApplier(objects, merger, symbols, layout).apply()
        return DylibEmitter(layout, relocated, merger, symbols, cpuType, cpuSubtype, installName).emit()
    }
    ...
    private class SymbolResolver(
        ...
        fun resolve(): SymbolResolver {
            ...
            // Undefined symbols that aren't defined anywhere become imports
    ...
    private class DylibLayout(
        ...
        val numTextSections = 1 + (if (hasRodata) 1 else 0)
        val numDataSections = if (hasData) 1 else 0
        val numSegments = 1 + (if (hasData) 1 else 0) // __TEXT + optional __DATA (no __PAGEZERO for dylib)
        val numLoadCmds: Int
        ...
        init {
            // LC_ID_DYLIB command size: base + padded install name string
            val idName = installName ?: "liboutput.dylib"
            val nameOffset = ID_DYLIB_CMD_BASE_SIZE
            val nameLen = idName.toByteArray(Charsets.US_ASCII).size + 1 // +1 for null
            val idDylibCmdSize = alignTo(nameOffset + nameLen, 8) // align to 8 bytes

            numLoadCmds = numSegments + 3 // segments + LC_ID_DYLIB + LC_SYMTAB + LC_DYSYMTAB

            loadCmdSize = textSegCmdSize +
                dataSegCmdSize +
                idDylibCmdSize +
                SYMTAB_CMD_SIZE +
                DYSYMTAB_CMD_SIZE
    ...
    private class RelocationApplier(
        ...
        private val layout: DylibLayout,
    ) {
        fun apply(): RelocatedSections {
            ...
            for ((objIdx, obj) in objects.withIndex()) {
                for (rel in obj.relocations) {
                    ...
                    val targetVaddr = symVaddrs[rel.symbol] ?: continue // skip unresolved (import)
        ...
        private fun applyRelocation(
            ...
            when (rel.type) {
                ...
                else -> {} // skip unsupported relocations for imports
            }
        }
    ...
    private class DylibEmitter(
        ...
        private fun emitHeader(buf: ByteArrayOutputStream) {
            ...
            writeU32(buf, MachO.MH_DYLIB)
            writeU32(buf, layout.numLoadCmds)
            writeU32(buf, layout.loadCmdSize)
            writeU32(buf, MachO.MH_DYLDLINK or MachO.MH_TWOLEVEL or MachO.MH_PIE or
                if (symbols.undefinedSymbols.isEmpty()) MachO.MH_NOUNDEFS else 0)
        ...
        private fun emitLoadCommands(
            ...
            // LC_SEGMENT_64 __TEXT (no __PAGEZERO for dylib)
            ...
            // LC_ID_DYLIB
            val idName = installName ?: "liboutput.dylib"
            val nameBytes = idName.toByteArray(Charsets.US_ASCII)
            val nameOffset = ID_DYLIB_CMD_BASE_SIZE
            val cmdSize = alignTo(nameOffset + nameBytes.size + 1, 8)
            writeU32(buf, MachO.LC_ID_DYLIB)
            writeU32(buf, cmdSize)
            writeU32(buf, nameOffset) // name offset within load command
            ...
            // Pad to alignment
            val written = nameOffset + nameBytes.size + 1
            for (i in written until cmdSize - (ID_DYLIB_CMD_BASE_SIZE - 4 * 3)) {
                // We already wrote 24 bytes of command header, need to pad the name
            }
            val totalWritten = 4 * 6 + nameBytes.size + 1 // 6 u32s + name + null
            val padNeeded = cmdSize - totalWritten
            if (padNeeded > 0) buf.write(ByteArray(padNeeded))

            // LC_SYMTAB
            writeU32(buf, MachO.LC_SYMTAB)
            writeU32(buf, SYMTAB_CMD_SIZE)
            writeU32(buf, layout.symtabFileOff)
            writeU32(buf, nsyms)
            writeU32(buf, strtabOff)
            writeU32(buf, strtabSize)

            // LC_DYSYMTAB — indices into the symbol table
            val nLocalSyms = 0
            ...
            writeU32(buf, nLocalSyms) // nlocalsym
            writeU32(buf, nLocalSyms) // iextdefsym
            writeU32(buf, nExtDefSyms) // nextdefsym
            writeU32(buf, nLocalSyms + nExtDefSyms) // iundefsym
            writeU32(buf, nUndefSyms) // nundefsym
            writeU32(buf, 0) // tocoff
            writeU32(buf, 0) // ntoc
            writeU32(buf, 0) // modtaboff
            writeU32(buf, 0) // nmodtab
            writeU32(buf, 0) // extrefsymoff
            writeU32(buf, 0) // nextrefsyms
            writeU32(buf, 0) // indirectsymoff
            writeU32(buf, 0) // nindirectsyms
            writeU32(buf, 0) // extreloff
            writeU32(buf, 0) // nextrel
            writeU32(buf, 0) // locreloff
            writeU32(buf, 0) // nlocrel
        }
        ...
        private fun buildSymbolTable(): List<SymEntry> {
            ...
            // Defined (exported) symbols first
            for ((name, resolved) in symbols.globalSymbols) {
